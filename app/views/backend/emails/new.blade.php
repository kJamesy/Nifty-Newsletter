        ...
        {{ Form::open(['url' => 'dashboard/emails/send', 'class' => 'form-horizontal']) }}  
            <div class="col-md-12 col-lg-9"> 
                <div class="form-group {{ $errors->first('subscribers') ? 'has-error' : '' }}">
                    ...
                    {{ Form::select('subscribers[]', $subscribers, $selected_subs, ['multiple' => true, 'class' => 'form-control select-fix', 'id' => 'subscribers']) }}                     
                </div> 
                <div class="form-group {{ $errors->first('mail_lists') ? 'has-error' : '' }}">
                    {{ Form::label('mail_lists', $errors->first('mail_lists'), ['class' => 'control-label']) }}
                    {{ Form::select('mail_lists[]', $mail_lists, $selected_lists, ['multiple' => true, 'class' => 'form-control select-fix', 'id' => 'mail_lists']) }}                       
                </div>                               
                ...
                <div class="form-group {{ $errors->first('email_body') ? 'has-error' : '' }}">
                    ...
                    {{ Form::textarea('email_body', Input::old('email_body'), ['id' => 'email_body', 'class' => 'form-control', 'rows' => '3'])}}              
                </div>   
            </div>
            <div class="col-md-12 col-lg-2 col-lg-offset-1">
                <div class="form-group">
                    {{ Form::label('tag_id', 'Email Tag', ['class' => 'control-label']) }}
                    {{ Form::select('tag_id', $tag_list, Input::old('tag_id'), ['class' => 'form-control', 'id' => 'tag_id']) }}
                </div> 
                <div class="form-group">
                    <div class="checkbox">
                        <label>
