...
@section('page-title') <h3><i class="fa fa-book"></i> Pages</h3> @stop
...
@section('page')
            ...
            <div class="col-sm-6 col-md-5">
                <div class="form-group">
                    <select name="bulk-options" id="bulk-options" class="form-control" disabled="disabled">
                        ...
                        <option value='1'>{{ $type == 'deleted' ? 'Destroy' : 'Delete' }}</option>
                        @if ( $type == 'deleted' )
                ...
                <div class="form-group">
                    <input type="hidden" id="bulkDeleteUrl" value = "{{  $type == 'deleted' ? URL::to('dashboard/pages/bulk-destroy') : URL::to('dashboard/pages/bulk-delete') }}" />
                    <input type="hidden" id="bulkRestoreUrl" value = "{{  $type == 'deleted' ? URL::to('dashboard/pages/bulk-restore') : '' }}" />
                    <div class="appendTarget"></div>
                </div>                
            </div>
            <div class="col-sm-6 col-md-4">                     
                <div class="form-group">
                    <button type="submit" class="btn btn-default btn-rect" id="bulk-submit" disabled="disabled" data-submittext="{{ $type == 'deleted' ? 'Permanently Delete ' : 'Trash ' }}">Submit
                    </button> 
                </div>
            </div>
        </div>
    ...
    <div class="col-md-4 hidden-xs hidden-sm">
        {{ Form::open(['url' => URL::to('dashboard/pages/set-records-per-page'), 'id' => 'set-records-form', 'class' => 'form-inline']) }}
            <div class="form-group">
    ...
    <div class="col-md-4 hidden-xs hidden-sm">
        {{ Form::open(['url' => URL::to('dashboard/pages/set-order-by'), 'id' => 'set-order-by-form', 'class' => 'form-inline']) }}
            <div class="form-group">
                ...
                <select name="order-by" id="order-by" class="form-control">
                    ...
                    <option value='3' {{ $orderBy == 3 ? 'selected' : '' }}>A-Z (Title)</option>
                    <option value='4' {{ $orderBy == 4 ? 'selected' : '' }}>Z-A (Title)</option>
                </select>
            </div>               
        {{ Form::close() }}
    </div>
    ...
    <script>
        jQuery(document).ready(function($) {
            ...
            function handleOption(option) {
                var submitText = $('#bulk-submit').data('submittext');
                switch(option) {
                    case "" :
                        $('#bulk-options-form').attr('action', '#');
                    ...
                    case "1" :
                        ...
                        $('#bulk-submit').removeAttr('disabled').removeClass().addClass('btn btn-default btn-rect btn-metis-1').text(submitText + ' ' + $(':checkbox.acheckbox:checked').size()+' Pages');
                        break;
                    case "2" :
                        $('#bulk-options-form').attr( 'action', $('#bulkRestoreUrl').val() ); 
                        $('#bulk-submit').removeAttr('disabled').removeClass().addClass('btn btn-default btn-rect btn-metis-5').text('Restore ' + $(':checkbox.acheckbox:checked').size()+' Pages');
                        break;                        
                }                                
            }
